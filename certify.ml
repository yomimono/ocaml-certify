let () = 
  ...
  let privkey = Nocrypto.Rsa.generate length (* should be user-set too, but keep in
                                          mind we may get Invalid_argument for
                                          nonsense lengths *) in
  (* can extract the public privkey with Nocrypto.rsa.pub_of_priv *)
  let csr = X509.CA.generate "my rad site" privkey in 
  let cert = sign (* at least let user change the valid period *) csr privkey issuer in
  let pem = X509.Encoding.Pem.to_pem_cstruct1 cert in
